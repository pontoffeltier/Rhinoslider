...
(function ($, window, undefined) {
	...
	$.fn.rhinoslider.effects = {
		...
		//options: direction, animateActive, easing, shiftValue, parts
		...
		chewyBars: function ($slider, params, callback) {
			var
				...
				slideBars = function ($slider, settings) {
					...
					settings.shiftValue = settings.shiftValue < 0 ? settings.shiftValue * -1 : settings.shiftValue;
					var vars = $slider.data('slider:vars');
					var
						partDuration,
					...
					switch(direction){
						case 'toTop':
							aniMap.left = 0;
							aniMap.top = -settings.shiftValue;
							cssMapNext.left = 0;
							cssMapNext.top = height + settings.shiftValue;
							values.width = width;
						...
						case 'toRight':
							...
							aniMap.top = 0;
							aniMap.right = -settings.shiftValue;
							cssMapNext.top = 0;
							cssMapNext.right = width + settings.shiftValue;
							break;
						case 'toBottom':
							values.width = width;
							values.height = vars.next.height() / settings.parts;
							aniMap.left = 0;
							aniMap.bottom = -settings.shiftValue;
							cssMapNext.left = 0;
							cssMapNext.bottom = height + settings.shiftValue;
							break;
						case 'toLeft':
							values.width = vars.next.width() / settings.parts;
							values.height = height;
							aniMap.top = 0;
							aniMap.left = -settings.shiftValue;
							cssMapNext.top = 0;
							cssMapNext.left = width + settings.shiftValue;
							break;
					}
					if (settings.animateActive) {
						$activeParts.each(function (i) {
	...
	$.fn.rhinoslider.preparations = {
		...
		chewyBars: function ($slider, settings, vars) {
			...
			if (shiftValue.indexOf(',') >= 0) {
				var tmp = shiftValue.split(',');
				settings.shiftValue = parseInt(tmp[0], 10);
			} else {
				settings.shiftValue = parseInt(shiftValue, 10);
			}
			var parts = String(tmpParts);
			if (parts.indexOf(',') >= 0) {
				var tmp = parts.split(',');
